...
var getTransactions = exports.getTransactions = function(userId, limit, page, sort, filters, callback) {
    ...
    async.parallel({
        results: function(cb) {
            ...
            var sortString = "ORDER BY t." + field + " " + order + ", t.created_at DESC LIMIT ?, ?;"
    ...
    async.parallel({
        results: function(cb) {
            db.runQuery(
                ...
                "  w.date, w.used, (SELECT COUNT(*) FROM debit d WHERE d.withdrawal_id = w.id) AS transaction_count, w.created_at, w.updated_at " +
                "FROM withdrawal w " +
                "WHERE user_id = ? " +
                "ORDER BY w." + field + " " + order + ", w.created_at DESC LIMIT ?, ?;",
