...
var getAvailableCashForCurrency = exports.getAvailableCashForCurrency = function(userId, currency, callback) {
    db.runQuery(
        "SELECT id, type, currency, amount, base_currency, base_amount, used " +
        "FROM withdrawal " +
...
var getSpendingForCountry = exports.getSpendingForCountry = function(tripId, country, callback) {
    db.runQuery(
        "SELECT SUM(base_amount) AS base_amount, count(id) AS num_transactions " +
        "FROM transaction " +
...
// TODO: Account for multiple currencies in one day
var getSpendingForDate = exports.getSpendingForDate = function(tripId, date, callback) {
    db.runQuery(
        "SELECT SUM(base_amount) AS base_amount, COUNT(id) AS num_transactions " +
        "FROM transaction " +
        "WHERE trip_id = ? AND country IS NOT NULL AND date = ?;",
        [tripId, date],
    callback);
};

var getSpendingForGraphByCategory = exports.getSpendingForGraphByCategory = function(tripId, startDate, endDate, callback) {
    db.runQuery(
        "SELECT c.category, SUM(t.base_amount) AS spent_base, SUM(t.amount) AS spent " +
        "FROM transaction t " +
...
var getSpendingForGraphByCountry = exports.getSpendingForGraphByCountry = function(tripId, startDate, endDate, callback) {
    db.runQuery(
        "SELECT country, SUM(base_amount) AS spent_base, SUM(amount) AS spent " +
        "FROM transaction " +
        "WHERE trip_id = ? " +
        "   AND date <= ? " +
        "   AND date >= ? " +
...
var getSpendingForGraph = exports.getSpendingForGraph = function(tripId, startDate, endDate, categories, callback) {
    db.runQuery(
        "SELECT date, SUM(base_amount) AS spent_base, SUM(amount) AS spent " +
	    "FROM transaction " +
        "WHERE trip_id = ? " +
        "   AND date <= ? " +
        "   AND date >= ? " +
        ...
        "GROUP BY date " +
        "ORDER BY date ASC;",
        [tripId, endDate, startDate, categories],
    callback);
};
...
var getTransactions = exports.getTransactions = function(userId, limit, page, sort, filters, callback) {
    ...
    async.parallel({
        results: function(cb) {
            ...
            console.log(filters);
            if(filters.tripId) {
                filterString += " AND t.trip_id = ? ";
                filterParams.push(filters.tripId);
            }
            ...
            if(filters.dateStart) {
                filterString += " AND t.date >= ? ";
                filterParams.push(filters.dateStart);
            }
            if(filters.dateEnd) {
                filterString += " AND t.date <= ? ";
                filterParams.push(filters.dateEnd);
            }
            if(filters.country) {
                if(filters.country === 'XX') {
                    filterString += " AND t.country IS NULL ";
                }
                else {
                    filterString += " AND t.country = ? ";
                    filterParams.push(filters.country);
                }
            }
            if(filters.categoryName) {
                filterString += " AND c.category = ? ";
                filterParams.push(filters.categoryName);
            }
...
var recordTransaction = exports.recordTransaction = function(userId, tripId, type, currency, amount, baseCurrency, baseAmount, date, categoryId, country, description, callback) {
    db.runQuery(
        "INSERT INTO transaction (user_id, trip_id, type, currency, amount, base_currency, base_amount, date, category_id, country, description, created_at) " +
        "VALUE(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP());",
        [userId, tripId, type.toUpperCase(), currency, amount, baseCurrency, baseAmount, date, categoryId, country, description],
    callback);
};
