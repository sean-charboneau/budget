...
var recordCreditTransaction = function(userId, amount, categoryId, country, currency, date, description, endDate, callback) {
    var baseCurrency;
    var exchangeRate;
    var type = 'credit';
    async.series([
        ...
        function(cb) {
            ...
            cashQuery.recordTransaction(userId, type, currency, amount, baseCurrency, baseAmount, date, categoryId, country, description, cb);
        },
        function(cb) {
            if(!baseCurrency || !exchangeRate) {
                return cb('Could not find base currency or exchange rate for user: ' + userId);
            ...
            async.whilst(
                ...
                function(innerCb) {
                    cashQuery.recordTransaction(userId, type, currency, amountPerDay, baseCurrency, baseAmountPerDay, rangeStart.format('YYYY-MM-DD'), categoryId, country, description, function(err, results) {
                        if(err) {
                            return innerCb(err);
                        }
                        rangeStart.add(1, 'd');
...
var recordSingleCashTransaction = function(userId, amount, categoryId, country, currency, date, description, baseCurrency, exchangeRate, callback) {
    var baseAmount = amount / exchangeRate;//FIX (should use rate from withdrawals)
    var type = 'cash';
    var transactionId;
    var updatedBaseAmount;
    async.series([
        function(cb) {
            cashQuery.recordTransaction(userId, type, currency, amount, baseCurrency, baseAmount, date, categoryId, country, description, function(err, results) {
                if(err) {
                    return cb(err);
                }
...
var recalculateCashTransactionValue = function(transactionId, callback) {
    ...
    async.series([
        function(cb) {
            cashQuery.getTransactionById(transactionId, function(err, results) {
                if(err) {
                    return cb(err);
                }
                if(!results || !results.length) {
                    return cb('No transaction found with id: ' + transactionId);
                }
        ...
        function(cb) {
            userController.getUserById(transaction.user_id, function(err, user) {
                if(err) {
                    return cb(err);
                }
                baseCurrency = user.base_currency;
        ...
        function(cb) {
            async.eachSeries(debits, function(debit, innerCb) {
                getDebitValue(debit.withdrawal_type, debit.debit_amount, debit.withdrawal_amount, debit.baseAmount, 
                transaction.date, transaction.currency, baseCurrency, function(err, value) {
                    if(err) {
                        return innerCb(err);
                    }
                    totalWorth += value;
...
var getDebitValue = function(withdrawalType, debitAmount, withdrawalAmount, withdrawalBaseAmount, transactionDate, transactionCurrency, userBaseCurrency, callback) {
    if(['ATM', 'EARNED', 'NEGATIVE'].indexOf(withdrawalType) == -1) {
        return callback('Invalid withdrawal type');
    }
...
var getExchangeRate(baseCurrency, currency, date, callback) {
...
var recordCashTransaction = function(userId, amount, categoryId, country, currency, date, description, endDate, callback) {
    var baseCurrency;
    var exchangeRate;
    async.series([
        function(cb) {
        ...
        function(cb) {
            ...
            recordSingleCashTransaction(userId, amount, categoryId, country, currency, date, description, baseCurrency, exchangeRate, cb);
        },
        function(cb) {
            if(!baseCurrency || !exchangeRate) {
                return cb('Could not find base currency or exchange rate for user: ' + userId);
            ...
            async.whilst(
                function() { return rangeStart <= rangeEnd },
                function(innerCb) {
                    recordSingleCashTransaction(userId, amountPerDay, categoryId, country, currency, rangeStart.format('YYYY-MM-DD'), description, baseCurrency, exchangeRate, function(err, results) {
                        if(err) {
                            return innerCb(err);
                        }
                        rangeStart.add(1, 'd');
...
var recordTransaction = exports.recordTransaction = function(userId, amount, categoryId, country, currency, date, description, endDate, type, callback) {
    ...
    if(type == 'credit') {
        return recordCreditTransaction(userId, amount, categoryId, country, currency, date, description, endDate, callback);
    }
    else if(type == 'cash') {
        return recordCashTransaction(userId, amount, categoryId, country, currency, date, description, endDate, callback);
    }
};

var recordEmptyWithdrawal = exports.recordEmptyWithdrawal = function(userId, amount, currency, date, callback) {
    cashQuery.recordWithdrawal(userId, currency, 0, null, 0, date, function(err, results) {
        if(err) {
            return callback(err);
        }
        if(!results) {
...
var recordWithdrawal = exports.recordWithdrawal = function(userId, amount, currency, date, isEarnedCash, isFee, feeAmount, callback) {
    ...
    async.series([
        ...
        function(cb) {
            ...
            cashQuery.recordWithdrawal(userId, currency, amount, baseCurrency, baseAmount, date, function(err, results) {
                if(err) {
                    return cb(err);
                }
                if(!results) {
